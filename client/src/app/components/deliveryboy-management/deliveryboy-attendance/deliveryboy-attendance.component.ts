...
import { ModalAttendanceComponent } from '../../Modals/modal-attendance/modal-attendance.component';
...
import { ModalAttendanceViewComponent } from '../../Modals/modal-attendance-view/modal-attendance-view.component';
...
@Component({
  selector: 'app-deliveryboy-attendance',
  templateUrl: './deliveryboy-attendance.component.html',
  styleUrls: ['./deliveryboy-attendance.component.css']
})
export class DeliveryboyAttendanceComponent implements OnInit {
  ...
  THeaders: any[] = [
    // { Key: 'Customer_Name', ShortKey: 'Customer_Name', Name: 'Name', If_Short: false, Condition: '' },
    ...
    { Key: 'TotalNoOfPresent', ShortKey: 'TotalNoOfPresent', Name: 'Total No Of Present', If_Short: false, Condition: '' },
    { Key: 'TotalNoOfAbsent', ShortKey: 'TotalNoOfAbsent', Name: 'Total No Of Absent', If_Short: false, Condition: '' },
  ];
  ...
  constructor(public ModalService: BsModalService,
              ...
              if (this.CurrDate.getHours() >= 12) {
                this.Session = 'Evening';
              } else {
                this.Session = 'Morning';
              }
              this.CurrDate = new Date(this.CurrDate.setHours(0, 0, 0, 0));
              ...
              this.AttendanceService.Attendance_SessionValidate({ Region: this.UserInfo.Region, CurrDate: this.CurrDate, Session: this.Session }).subscribe(response => {
                if (response.Status && response.Status === true) {
                  this.SessionValidList = response.Response;
                  if (this.SessionValidList === null) {
  ...
  Service_Loader() {
    ...
    this.TableLoader();
    this.AttendanceService.All_DeliveryPerson_AttendanceList(Data).subscribe(response => {
  ...
  CreateAttendance() {
    ...
    this.modalReference = this.ModalService.show(ModalAttendanceComponent,
  ...
  ViewAttendance(index: any) {
    const initialState = {
      Type: 'View',
      DeliveryPersonId: this.CollectionDetails[index]._id
    };
    this.modalReference = this.ModalService.show(ModalAttendanceViewComponent,
      Object.assign({ initialState }, { ignoreBackdropClick: true, class: 'modal-lg modal-dialog-centered animated zoomIn' }));
   //  this.modalReference.content.onClose.subscribe(response => {
   //    if (response.Status) {
